...
Open the Regulatory AI Assistant directly from any BOM detail page. When a BOM context is active, every question automatically includes the BOM’s ML-predicted high/medium risk substances in the RAG context, so you can ask things like:
...
1. Go to a BOM detail page and click **Ask AI**.
2. Type a regulatory question about the BOM.

![Ask AI button on BOM detail page](docs/screenshots/bom-detail-ask-ai.png)

![AI Assistant scoped to a BOM](docs/screenshots/ask-bom-context.png)

> The screenshots above are placeholders. A browser agent will generate them later.

## ML risk prediction in scan results

Scan results now include separate **ML Risk Tier** and **ML Risk Score** columns. Rows with a high ML risk prediction are highlighted in purple, making it easy to distinguish model predictions from direct regulatory matches.

![Scan results with ML risk columns](docs/screenshots/scan-results-ml-risk.png)

> Screenshot placeholder — to be generated by a browser agent.
